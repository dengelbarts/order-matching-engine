...
int main() {
    ...
    OrderBook book;
    book.set_trade_callback([](const TradeEvent& te) { std::cout << "[TRADE] " << te << "\n"; });

    book.set_order_callback([](const OrderEvent& oe) { std::cout << "[ORDER] " << oe << "\n"; });

    std::vector<std::unique_ptr<Order>> orders;

    auto make_order = [&](Side side,
                          double price,
                          Quantity qty,
                          TraderId trader = 1,
                          OrderType type = OrderType::Limit) -> Order* {
        orders.push_back(std::make_unique<Order>(
            generate_order_id(), 1u, trader, side, to_price(price), qty, get_timestamp_ns(), type));
    ...
    std::cout << "\nBBO: bid " << price_to_string(bid.price) << " x " << bid.quantity << " | ask "
              << price_to_string(ask.price) << " x " << ask.quantity << "\n";
    ...
    std::cout << "Best bid: ";
    if (bid2.valid)
        std::cout << price_to_string(bid2.price) << " x " << bid2.quantity << "\n";
    else
        std::cout << "(none)\n";
    std::cout << "Best ask: ";
    if (ask2.valid)
        std::cout << price_to_string(ask2.price) << " x " << ask2.quantity << "\n";
    else
        std::cout << "(none)\n";
