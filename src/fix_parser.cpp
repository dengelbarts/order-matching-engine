...
ParsedMessage parse_fix_message(std::string_view msg)
{
    ...
    {
        case FIXMsgType::New:
            if (!result.has_side) { result.error = "missing side"; return result; }
            if (!result.has_price) { result.error = "missing price"; return result; }
            if (!result.has_qty) { result.error = "missing qty"; return result; }
            if (result.qty <= 0) { result.error = "qty must be > 0"; return result; }
            if (result.price <= 0) { result.error = "price must be > 0"; return result; }
            break;
        
        case FIXMsgType::Cancel:
            if (!result.has_id) { result.error = "missing id"; return result; }
            break;
