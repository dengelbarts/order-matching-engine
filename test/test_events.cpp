...
protected:
    ...
    void SetUp() override {
        book.set_trade_callback([this](const TradeEvent& te) { trade_events.push_back(te); });
        book.set_order_callback([this](const OrderEvent& oe) { order_events.push_back(oe); });
    }
    ...
    Order* create_order(Side side,
        ...
        owned_.push_back(std::make_unique<Order>(
            generate_order_id(), symbol, trader, side, price, qty, get_timestamp_ns(), type));
        return owned_.back().get();
    }

    size_t count_event_type(OrderEventType t) const {
        return static_cast<size_t>(std::count_if(order_events.begin(),
                                                 order_events.end(),
                                                 [t](const OrderEvent& e) { return e.type == t; }));
    }
};
...
TEST_F(EventTest, IntegrationTest220Orders) {
    ...
    for (int i = 0; i < 10; ++i) {
        buys.push_back(std::make_unique<Order>(generate_order_id(),
                                               symbol,
                                               1,
                                               Side::Buy,
                                               P1000,
    ...
    for (int i = 0; i < 10; ++i) {
        sells.push_back(std::make_unique<Order>(generate_order_id(),
                                                symbol,
                                                2,
                                                Side::Sell,
                                                P1100,
    ...
    Order agg_sell(generate_order_id(),
                   symbol,
                   3,
                   Side::Sell,
                   P1000,
    ...
    Order agg_buy(generate_order_id(),
                  symbol,
                  4,
                  Side::Buy,
                  P1100,
