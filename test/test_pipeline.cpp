...
TEST(MatchingPipelineTest, TradeCallbackFires) {
    ...
    pipeline.set_trade_callback(
        [&](const TradeEvent&) { trade_count.fetch_add(1, std::memory_order_relaxed); });
    pipeline.start();

    pipeline.submit(
        OrderCommand::make_new(1, 1, 1, Side::Buy, to_price(100.0), 100, 0, OrderType::Limit));
    pipeline.submit(
        OrderCommand::make_new(2, 1, 2, Side::Sell, to_price(100.0), 100, 0, OrderType::Limit));
...
TEST(MatchingPipelineTest, HundredKOrdersThroughPipeline) {
    ...
    for (int i = 1; i <= N; ++i) {
        ...
        TraderId trader =
            (side == Side::Buy) ? 1 : 2; // different traders — avoid self-match prevention
        pipeline.submit(OrderCommand::make_new(
...
TEST(MatchingPipelineTest, CleanShutdownNoLostOrders) {
    ...
    for (int i = 1; i <= N; ++i)
        pipeline.submit(OrderCommand::make_new(static_cast<OrderId>(i),
                                               1,
                                               1,
                                               Side::Buy,
                                               to_price(100.0) + i,
