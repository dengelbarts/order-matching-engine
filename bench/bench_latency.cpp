...
static void print_stats(const char* label, const std::vector<int64_t>& sorted_lat) {
    ...
    int64_t total = std::accumulate(sorted_lat.begin(), sorted_lat.end(), int64_t{0});
...
static void
populate_book(OrderBook& book, int n, Price mid, Price tick, std::mt19937& rng, uint64_t id_start) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (p <= 0)
            p = tick;
        book.create_order(id_start + static_cast<uint64_t>(i),
                          1,
                          1,
                          s,
                          p,
...
int main() {
    ...
    // ── Warmup ──────────────────────────────────────────────────────────────
    {
        ...
        for (int i = 0; i < WARMUP; ++i)
            book.create_order(static_cast<uint64_t>(i + 1),
                              1,
                              1,
                              Side::Buy,
                              MID - TICK,
    ...
    // ── 2a: Limit add under load (non-crossing, measures pure insertion) ────
    {
        ...
        for (int i = 0; i < MEASURED; ++i) {
            ...
            book.create_order(
                base + static_cast<uint64_t>(i), 1, 1, s, p, 100, 0, OrderType::Limit);
    ...
    // ── 2b: IOC match under load (measures match path with depth) ────────────
    {
        ...
        for (int i = 0; i < MEASURED; ++i) {
            ...
            auto t0 = std::chrono::high_resolution_clock::now();
            book.create_order(base + static_cast<uint64_t>(i), 1, 1, s, MID, 1, 0, OrderType::IOC);
            auto t1 = std::chrono::high_resolution_clock::now();
