...
static void execute_workload(OrderBook& book, const std::vector<WorkloadOp>& ops)
{
    for (const auto& op : ops) {
        ...
        case WorkloadOp::Type::AddLimit:
            book.create_order(op.order_id, 1, 1, op.side, op.price,
                              op.quantity, 0, OrderType::Limit);
            break;
...
static void BM_CancelHeavy(benchmark::State& state)
{
    ...
    for (int i = 0; i < N; ++i) {
        ...
        if (r < 0.60 || live.empty()) {
            ...
            ops.push_back({CancelOp::T::Add, next, s, p,
            ...
            if (p <= 0) p = TICK;
            Side s = (dice(rng) < 0.5) ? Side::Buy : Side::Sell;
            ops.push_back({CancelOp::T::IOC, next++, s, p,
                           static_cast<Quantity>(qty_dist(rng))});
        }
    }
    ...
    for (auto _ : state) {
        ...
        for (const auto& op : ops) {
            switch (op.type) {
            case CancelOp::T::Add:
                book.create_order(op.id, 1, 1, op.side, op.price,
                                  op.qty, 0, OrderType::Limit);
                break;
...
static void BM_DeepBook(benchmark::State& state)
{
    ...
    for (auto _ : state) {
        OrderBook book;
        for (int i = 0; i < LEVELS; ++i)
            book.create_order(ids[i], 1, 1, Side::Sell, prices[i],
                              100, 0, OrderType::Limit);

        // Market buy sweeps the entire ask side
        book.create_order(sweep_id, 1, 2, Side::Buy, 0,
