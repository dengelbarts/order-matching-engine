...
struct Soak {
    ...
    Price rand_price() { return std::max(MID + static_cast<Price>(tick_d(rng)) * TICK, TICK); }

    Quantity rand_qty() { return static_cast<Quantity>(qty_d(rng)); }
    ...
    void step() {
        ...
        if (r < 50 || live.empty()) {
            ...
            Order* rest = book.create_order(
                next_id, 1, trader, s, rand_price(), rand_qty(), 0, OrderType::Limit);
            ...
            Side s = (rng() & 1) ? Side::Buy : Side::Sell;
            book.create_order(next_id++, 1, 21, s, rand_price(), rand_qty(), 0, OrderType::IOC);
            ...
            // FOK — uses trader 22
            Side s = (rng() & 1) ? Side::Buy : Side::Sell;
            book.create_order(next_id++, 1, 22, s, rand_price(), rand_qty(), 0, OrderType::FOK);
        }
    ...
    while (true) {
        ...
        if (now >= t_next_print) {
            ...
            const double ops_per_s =
                (delta_s > 0.0) ? static_cast<double>(soak.ops - ops_snapshot) / delta_s : 0.0;
